...
##########################################################################
#### Aux functions - copied as is from test_frcnn.py
##########################################################################
...
def predit_images(img_names, img_base_path, model_config, model_path, bbox_threshold=0.8):
    ...
    # Sanity check - check all images exists
    for img in img_names:
        if not os.path.isfile(img_base_path + '/' + img):
            print("cant locate {}".format(img))
            assert 0

    # load model
    model_dict = load_model(model_config, model_path)
    C = model_dict['C']
