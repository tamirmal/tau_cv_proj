...
def vgg_prepare_features_for_train_v2(img_list, Y, desired_length = None):
    """
    Input :
        img_list : list of dicts containing img name and label
    ...
    # Extract features over original image set - to ensure that an original image wont be missed in augmentation
    features_list = []
    for img in img_list:
        img_path = img['name']
        assert img['class'] is not None
    ...
    # Now augment and extract
    print("Applying data-augmentation")
    # Image show be in array format, generate list of arrays
    X = []
    for img in img_list:
